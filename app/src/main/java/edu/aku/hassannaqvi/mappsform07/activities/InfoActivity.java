...
public class InfoActivity extends Activity {
    ...
    static Map<String, FUPContract> childMap;

    ArrayList<String> partNames;
    int position;
    @BindView(R.id.fldGrpmp08a004a)
    LinearLayout fldGrpmp08a004a;
    @BindView(R.id.motherName)
    TextView motherName;
    Boolean flagForm9_10;
    @BindView(R.id.fupRound)
    TextView fupRound;
    Boolean rebackFlag;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mp08a004.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {

                if (i > 0) {
                    mp08a002.setText(childMap.get(mp08a004.getSelectedItem().toString()).getSno());
...
//        Reback intent
        rebackFlag = getIntent().getBooleanExtra("reBack", false);

        if (rebackFlag) {
            for (int i = 0; i < LHWsName.size(); i++) {
                if (LHWsName.get(i).equals(AppMain.fc.getLhwCode())) {
                    lhws.setSelection(i);
                    break;
                }
            }
            lhws.setEnabled(false);

            mp08a001.setText(AppMain.fc.getHousehold());
            mp08a001.setEnabled(false);

            onCheckParticipantsClick();
        }
    ...
    public int populateChildSpinner() {
        ...
        if (enrolledParticipant.size() != 0) {
            ...
            if (flagForm9_10) {
                ...
                if (rebackFlag) {
                    ...
                    childSize = populateChildSpinner();
                    if (childSize > 0) {
